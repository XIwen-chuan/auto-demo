...
const filterSlotsCandidates = computed(() => {
  ...
  // 对 candidates 去重
  const candidatesDeduped: models.Emit[] = [];
  candidates.forEach((candidate) => {
    if (
      !candidatesDeduped.find(
        (candidateDeduped) => candidateDeduped.id === candidate.id
      )
    ) {
      candidatesDeduped.push(candidate);
    }
  });
  console.log("candidatesDeduped", candidatesDeduped);
  return candidatesDeduped;
});
